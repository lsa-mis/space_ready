...
RSpec.describe RoomPolicy, type: :policy do
  ...
  context 'with rover role' do
    ...
    it { is_expected.to permit_only_actions(%i[is_rover delete_image upload_images]) }
  end

  context 'with readonly role' do
    subject { described_class.new({ user: user, role: "readonly" }, room) }

    it { is_expected.to forbid_actions(%i[is_rover is_developer is_admin index new create archive unarchive destroy delete_image upload_images]) }
  ...
  context 'with admin role' do
    ...
    it { is_expected.to permit_only_actions(%i[is_admin index show create new archive unarchive destroy delete_image upload_images]) }
  end
  ...
  context 'with developer role' do
    ...
    it { is_expected.to permit_only_actions(%i[is_admin is_developer index show create new archive unarchive destroy delete_image upload_images]) }
  end
